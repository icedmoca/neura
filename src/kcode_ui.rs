//! Launcher for the local Kcode cockpit UI (`scripts/kcodeui`).
//!
//! All slash-command / skill entry points (`/kcodeui` in the TUI, the REPL skill
...
/// Default port the launcher prefers; kept in sync with `scripts/kcode-ui-server.py`.
pub const DEFAULT_PORT: u16 = 8768;

fn script_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("scripts/kcodeui")
}
...
/// Start (or reuse) the Kcode UI server and return a human-readable status line
/// that always points at a URL the user can actually open.
pub fn launch() -> String {
    match try_launch() {
        Ok(url) => format!("Kcode UI is running: {url}\nLive state API: {url}/api/state"),
        Err(msg) => msg,
    }
}
...
fn try_launch() -> Result<String, String> {
    ...
    if !script.exists() {
        return Err(format!(
            "Kcode UI launcher not found at {}.\nRun `scripts/kcodeui` from the repo root, then open http://127.0.0.1:{DEFAULT_PORT}.",
            script.display()
        ));
    }
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "Kcode UI launcher produced no output stream.".to_string())?;

    // Read the server's announced `KCODE_UI_URL=...` on a detached thread so the
    ...
    std::thread::spawn(move || {
        ...
        for line in reader.lines().map_while(Result::ok) {
            if let Some(url) = line.trim().strip_prefix("KCODE_UI_URL=") {
    ...
    match rx.recv_timeout(Duration::from_secs(4)) {
        ...
        Err(_) => Ok(format!(
            "http://127.0.0.1:{DEFAULT_PORT} (still starting — give it a moment, or run `scripts/kcodeui` from the repo root to watch logs)"
        )),
    }
}
