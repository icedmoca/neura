...
fn with_clean_provider_test_env<T>(f: impl FnOnce() -> T) -> T {
    ...
    let prev_subscription =
        std::env::var_os(crate::subscription_catalog::NEURA_SUBSCRIPTION_ACTIVE_ENV);
    ...
    if let Some(prev_subscription) = prev_subscription {
        crate::env::set_var(
            crate::subscription_catalog::NEURA_SUBSCRIPTION_ACTIVE_ENV,
            prev_subscription,
        );
    } else {
        crate::env::remove_var(crate::subscription_catalog::NEURA_SUBSCRIPTION_ACTIVE_ENV);
    }
    crate::subscription_catalog::clear_runtime_env();
    result
}
