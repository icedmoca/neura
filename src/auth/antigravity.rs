...
mod tests {
    ...
    fn build_auth_url_includes_antigravity_scope_and_redirect() {
        ...
        crate::env::set_var(CLIENT_ID_ENV, "test-antigravity-client-id");
        ...
        assert!(url.contains("client_id=test-antigravity-client-id"));
    ...
    #[test]
    fn build_auth_url_has_empty_client_id_when_env_missing() {
        let _guard = lock_test_env();
        crate::env::remove_var(CLIENT_ID_ENV);
        let url = build_auth_url(
            "http://127.0.0.1:51121/oauth-callback",
    ...
    #[test]
    fn blank_env_vars_fall_back_to_empty_credentials() {
        let _guard = lock_test_env();
        crate::env::set_var(CLIENT_ID_ENV, "   ");
        crate::env::set_var(CLIENT_SECRET_ENV, "   ");

        assert_eq!(antigravity_client_id(), "");
