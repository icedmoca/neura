...
fn main() -> anyhow::Result<()> {
    let out_dir = std::env::args()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/tmp/neura_bench"));
    ...
    for task in &tasks {
        ...
        results.push(BenchResult {
            ...
            ablation_delta,
            promotion_allowed,
            evaluator_version: "neura-bench-v2".into(),
        });
    }
    ...
    let summary = BenchSummary {
        ...
        binary_version: command_output("/home/dad/.neura/bin/neura", &["--version"]),
...
fn command_output(cmd: &str, args: &[&str]) -> String {
    std::process::Command::new(cmd)
        .args(args)
        .current_dir("/home/dad/.neura/build-src/neura")
        .output()
        .ok()
