...
pub struct NeuraTokenCompressionLink {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct NeuraExecutionOutcome {
    pub directive_id: String,
...
struct RankedDirective<'a> {
    directive: &'a NeuraDirective,
    score: f64,
}

#[derive(Debug, Default, Clone, Serialize, Deserialize, PartialEq)]
pub struct NeuraMemoryStore {
    #[serde(default)]
    pub directives: Vec<NeuraDirective>,
    #[serde(default)]
    pub token_compression_links: Vec<NeuraTokenCompressionLink>,
    #[serde(default)]
    pub execution_outcomes: Vec<NeuraExecutionOutcome>,
}
...
pub fn memory_path() -> PathBuf {
    neura_home()
        .join("self_memory")
        .join("neura_directives.json")
}

pub fn ingest_text(source: impl Into<String>, text: &str) -> io::Result<Vec<NeuraDirective>> {
    let extracted = extract_neura_directives(source.into(), text);
...
pub fn record_execution_outcome(
    ...
    store.execution_outcomes.push(NeuraExecutionOutcome {
...
pub fn prompt_memory_block() -> Option<String> {
    ...
    let mut lines = vec![
        "\n# Dynamic .neura memory".to_string(),
        "The user wants instructions or intent containing `.neura` to persistently improve Neura behavior. Treat these as adaptive memory nodes unless they conflict with safety, security, or explicit later instructions.".to_string(),
    ...
    if let Ok(cognition_nodes) =
        adaptive_cognition::retrieve_for_prompt(".neura adaptive cognition memory directives", 900)
    {
    ...
    if let Ok(report) =
        adaptive_cognition::run_synthetic_scientific_governance("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) = adaptive_cognition::run_cognitive_context_economy("prompt_memory_block") {
        lines.push(report.prompt_status);
    }

    if let Ok(report) =
        adaptive_cognition::run_hierarchical_epistemic_context("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) = adaptive_cognition::run_emergent_quality_coherence("prompt_memory_block") {
        lines.push(report.prompt_status);
    }

    if let Ok(report) = adaptive_cognition::run_cognitive_substrate_synthesis("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) =
        adaptive_cognition::run_substrate_convergence_stabilization("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) =
        adaptive_cognition::run_structure_discovery_representation("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) = adaptive_cognition::run_adaptive_complexity_collapse("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) =
        adaptive_cognition::run_active_representation_evolution("prompt_memory_block")
    {
        lines.push(report.prompt_status);
    }

    if let Ok(report) = adaptive_cognition::run_emergent_strategy_discovery("prompt_memory_block") {
        lines.push(report.prompt_status);
    }
...
fn extract_neura_directives(source: String, text: &str) -> Vec<NeuraDirective> {
    if !text.to_ascii_lowercase().contains(".neura") {
        return Vec::new();
    }

    let now = Utc::now();
    text.lines()
        .filter(|line| line.to_ascii_lowercase().contains(".neura"))
        .map(str::trim)
        .filter(|line| !line.is_empty())
        .map(|line| {
            let token_count_estimate = estimate_token_count(line);
            NeuraDirective {
                id: format!("neura-{}", now.timestamp_nanos_opt().unwrap_or_default()),
                created_at: now,
                source: source.clone(),
...
fn project_into_memory_graph(
    store_path: &Path,
    store: &mut NeuraMemoryStore,
    directives: &[NeuraDirective],
) -> io::Result<()> {
    let graph_path = store_path.with_file_name("neura_memory_graph.json");
    ...
    for directive in directives {
        ...
        store
            ...
            .push(NeuraTokenCompressionLink {
...
fn project_into_adaptive_cognition(directives: &[NeuraDirective]) -> io::Result<()> {
    for directive in directives {
        let mut provenance = BTreeMap::new();
        provenance.insert("neura_directive_id".to_string(), directive.id.clone());
...
fn rank_directives(store: &NeuraMemoryStore) -> Vec<RankedDirective<'_>> {
    let now = Utc::now();
...
fn directive_score(directive: &NeuraDirective, now: DateTime<Utc>) -> f64 {
...
fn temporal_decay_factor(directive: &NeuraDirective, now: DateTime<Utc>) -> f64 {
...
fn recompute_adaptive_scores(store: &mut NeuraMemoryStore) {
...
fn execution_outcome_totals(outcomes: &[NeuraExecutionOutcome]) -> HashMap<String, f64> {
...
fn contradiction_score(directive: &NeuraDirective, directives: &[NeuraDirective]) -> f64 {
...
fn estimate_token_count(text: &str) -> usize {
    ...
    // Keeps .neura memory linked to compaction budgets without requiring model IO.
...
fn infer_tags(text: &str) -> Vec<String> {
    let lower = text.to_ascii_lowercase();
    let mut tags = vec![".neura".to_string()];
    for (needle, tag) in [
        ("remember", "memory"),
...
fn normalize_store(store: &mut NeuraMemoryStore) {
    for directive in &mut store.directives {
...
fn load_store_from_path(path: &Path) -> io::Result<NeuraMemoryStore> {
    match fs::read_to_string(path) {
        Ok(contents) => serde_json::from_str(&contents)
            .map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err)),
        Err(err) if err.kind() == io::ErrorKind::NotFound => Ok(NeuraMemoryStore::default()),
        Err(err) => Err(err),
    }
}

fn save_store_to_path(path: &Path, store: &NeuraMemoryStore) -> io::Result<()> {
...
mod tests {
    ...
    #[test]
    fn extracts_only_neura_lines() {
        let directives = extract_neura_directives(
            "test".to_string(),
            "ignore\nremember .neura recursively\nother",
        );
        assert_eq!(directives.len(), 1);
        assert_eq!(directives[0].content, "remember .neura recursively");
    ...
    fn normalizes_for_deduplication() {
        assert_eq!(normalize("Remember   .NEURA"), normalize("remember .neura"));
    }

    #[test]
    fn repeated_directives_reinforce_instead_of_duplicate() {
        let now = Utc::now();
        let mut store = NeuraMemoryStore {
            directives: vec![NeuraDirective {
                id: "one".to_string(),
                created_at: now,
                source: "test".to_string(),
                content: "remember .neura memory".to_string(),
        ...
        reinforce_matching_directive(&mut store, "remember .neura memory", 0.2, now);
    ...
    fn contradiction_scoring_detects_opposing_directives() {
        let now = Utc::now();
        let older = NeuraDirective {
            id: "a".to_string(),
            created_at: now,
            source: "test".to_string(),
            content: "always remember .neura graph memory".to_string(),
            tags: infer_tags("always remember .neura graph memory"),
        ...
        let newer = NeuraDirective {
            content: "never remember .neura graph memory".to_string(),
            id: "b".to_string(),
            ..older.clone()
        };
    ...
    fn directive_score_uses_outcome_and_contradiction() {
        let now = Utc::now();
        let mut directive =
            extract_neura_directives("test".to_string(), ".neura use execution outcome linkage")
                .remove(0);
