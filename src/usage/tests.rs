...
#[test]
fn test_parse_openai_usage_payload_uses_exact_wham_counts_when_available() {
    let json = serde_json::json!({
        "rate_limit": {
            "allowed": true,
            "primary_window": {
    ...
    assert_eq!(parsed.limits[0].name, "5-hour window");
    assert!((parsed.limits[0].usage_percent - 5.456).abs() < 0.001);
    assert_eq!(parsed.limits[1].name, "7-day window");
    assert!((parsed.limits[1].usage_percent - 12.345).abs() < 0.001);
}
