...
fn parse_wham_window(window: &serde_json::Value, name: &str) -> Option<UsageLimit> {
    ...
    let used_percent =
        parse_exact_usage_percent_from_counts(obj).or_else(|| parse_usage_percent_from_obj(obj))?;
...
fn parse_exact_usage_percent_from_counts(
    obj: &serde_json::Map<String, serde_json::Value>,
) -> Option<f32> {
    let used = obj
        .get("used")
        .or_else(|| obj.get("used_tokens"))
        .or_else(|| obj.get("used_count"))
        .or_else(|| obj.get("current"))
        .and_then(parse_f32_value);
    let limit = obj
        .get("limit")
        .or_else(|| obj.get("token_limit"))
