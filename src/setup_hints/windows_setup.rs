...
fn create_hotkey_shortcut(use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::kcode_dir()?.join("hotkey");
    ...
    let _ = std::process::Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
            "Get-Process powershell, pwsh -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*kcode-hotkey*' } | Stop-Process -Force -ErrorAction SilentlyContinue",
        ])
        .output();

    let ps1_path = hotkey_dir.join("kcode-hotkey.ps1");
    let ps1_content = format!(
        r#"# kcode Alt+; global hotkey listener
# Auto-generated by kcode setup-hotkey. Runs at login via startup shortcut.
...
"#,
    ...
    let vbs_path = hotkey_dir.join("kcode-hotkey-launcher.vbs");
...
$shortcut = $shell.CreateShortcut("{startup_dir}\kcode-hotkey.lnk")
$shortcut.TargetPath = "wscript.exe"
$shortcut.Arguments = '"{vbs_path}"'
$shortcut.Description = "kcode Alt+; hotkey listener"
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up Alt+; to launch kcode from anywhere?\x1b[0m              \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens kcode in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1mkcode setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for kcode\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkey updated: \x1b[1mAlt+;\x1b[0m → Alacritty + kcode"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m from anywhere to launch kcode.                \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting kcode normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up \x1b[1mAlt+;\x1b[0m → {} + kcode...",
        terminal_name
    );
...
pub(super) fn create_windows_desktop_shortcut(state: &mut SetupHintsState) -> Result<()> {
    ...
    let shortcut_path = format!("{}\\Desktop\\kcode.lnk", desktop_dir);
...
$shortcut.Description = "kcode - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
