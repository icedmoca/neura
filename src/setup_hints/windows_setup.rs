...
fn create_hotkey_shortcut(use_alacritty: bool) -> Result<()> {
    ...
    let hotkey_dir = storage::neura_dir()?.join("hotkey");
    ...
    let _ = std::process::Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
            "Get-Process powershell, pwsh -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*neura-hotkey*' } | Stop-Process -Force -ErrorAction SilentlyContinue",
        ])
        .output();

    let ps1_path = hotkey_dir.join("neura-hotkey.ps1");
    let ps1_content = format!(
        r#"# neura Alt+; global hotkey listener
# Auto-generated by neura setup-hotkey. Runs at login via startup shortcut.
...
"#,
    ...
    let vbs_path = hotkey_dir.join("neura-hotkey-launcher.vbs");
...
$shortcut.Description = "neura Alt+; hotkey listener"
...
fn nudge_hotkey(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Set up Alt+; to launch neura from anywhere?\x1b[0m              \x1b[36m│\x1b[0m"
    );
    eprintln!(
    ...
    eprintln!(
        "\x1b[36m│\x1b[0m    Opens neura in {:<39}    \x1b[36m│\x1b[0m",
        ...
        "y" | "yes" => {
            ...
            match create_hotkey_shortcut(using_alacritty) {
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "    You can set it up manually later with: \x1b[1mneura setup-hotkey\x1b[0m"
                    );
                    eprintln!();
                    false
                }
...
fn nudge_alacritty(state: &mut SetupHintsState) -> bool {
    ...
    eprintln!("\x1b[36m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
    eprintln!(
        "\x1b[36m│\x1b[0m \x1b[1m💡 Alacritty: the fastest terminal for neura\x1b[0m               \x1b[36m│\x1b[0m"
    );
    eprintln!(
        "\x1b[36m│\x1b[0m                                                             \x1b[36m│\x1b[0m"
    );
                ...
                Ok(()) => {
                    ...
                    if state.hotkey_configured {
                        ...
                        match create_hotkey_shortcut(true) {
                            Ok(()) => {
                                eprintln!(
                                    "  \x1b[32m✓\x1b[0m Hotkey updated: \x1b[1mAlt+;\x1b[0m → Alacritty + neura"
                                );
                            }
                            Err(e) => {
...
fn prompt_try_it_out(installed_alacritty: bool) {
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    Press \x1b[1mAlt+;\x1b[0m from anywhere to launch neura.                \x1b[32m│\x1b[0m"
    );
    ...
    eprintln!(
        "\x1b[32m│\x1b[0m    \x1b[90m(Starting neura normally in 3 seconds...)\x1b[0m                 \x1b[32m│\x1b[0m"
    );
    eprintln!("\x1b[32m└─────────────────────────────────────────────────────────────┘\x1b[0m");
    eprintln!();
...
pub(super) fn run_setup_hotkey_windows() -> Result<()> {
    ...
    eprintln!(
        "  Setting up \x1b[1mAlt+;\x1b[0m → {} + neura...",
        terminal_name
    );
...
$shortcut.Description = "neura - AI coding agent"
$shortcut.Save()
Write-Output "OK"
"#,
