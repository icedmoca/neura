...
//! This lets iOS/web clients interact with neura sessions identically
//! to TUI clients.
//!
//! Architecture:
...
// ---------------------------------------------------------------------------
// Device registry (persisted to ~/.neura/devices.json)
// ---------------------------------------------------------------------------
...
impl DeviceRegistry {
    /// Load from ~/.neura/devices.json
    pub fn load() -> Self {
        let path = match storage::neura_dir() {
    ...
    /// Save to ~/.neura/devices.json
    pub fn save(&self) -> Result<()> {
        let path = storage::neura_dir()?.join("devices.json");
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `neura pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "neura",
        "server_version": env!("NEURA_VERSION"),
    });
    http_response(200, "OK", &body.to_string())
}
