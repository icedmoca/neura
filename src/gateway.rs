...
//! This lets iOS/web clients interact with kcode sessions identically
//! to TUI clients.
//!
//! Architecture:
...
// ---------------------------------------------------------------------------
// Device registry (persisted to ~/.kcode/devices.json)
// ---------------------------------------------------------------------------
...
impl DeviceRegistry {
    /// Load from ~/.kcode/devices.json
    pub fn load() -> Self {
        let path = match storage::kcode_dir() {
    ...
    /// Save to ~/.kcode/devices.json
    pub fn save(&self) -> Result<()> {
        let path = storage::kcode_dir()?.join("devices.json");
...
async fn handle_pair_request(
    ...
    // Reload from disk - pairing codes are generated by `kcode pair` CLI
    *reg = DeviceRegistry::load();
    ...
    let body = serde_json::json!({
        "token": token,
        "server_name": "kcode",
        "server_version": env!("KCODE_VERSION"),
    });
    http_response(200, "OK", &body.to_string())
}
