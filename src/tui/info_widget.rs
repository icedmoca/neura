...
impl WidgetKind {
    ...
    pub fn priority(self) -> u8 {
        match self {
            WidgetKind::ModelInfo => 0, // IL/model stats should stay visible and stable.
            WidgetKind::Diagrams => 1,  // User explicitly wants to see it.
            WidgetKind::WorkspaceMap => 2,
            WidgetKind::Overview => 3,
            WidgetKind::Todos => 4,
            WidgetKind::ContextUsage => 5,
            WidgetKind::UsageLimits => 6, // Important when near limits.
    ...
    pub fn preferred_side(self) -> Side {
        match self {
            ...
            WidgetKind::ModelInfo => Side::Right,
    ...
    pub fn min_height(self) -> u16 {
        match self {
            ...
            WidgetKind::ModelInfo => 12, // IL/model stats + limits + memory need stable space.
            WidgetKind::Tips => 3,
...
pub(crate) fn calculate_widget_height(
    ...
    let content_height = match kind {
        ...
        WidgetKind::ModelInfo => {
            ...
            if crate::interlang::enabled() {
                h += 3; // Interlang cumulative + latest + estimated savings lines
            }
            ...
            if data.usage_info.is_some() {
                h += 1; // Limits section divider inside IL stats.
            }
            if let Some(info) = &data.memory_info
                && (info.total_count > 0 || info.activity.is_some() || info.sidecar_model.is_some())
            {
                h += 1; // Memory section divider inside IL stats.
                h += 3; // Header, count/status, model/trace summary.
                if info.activity.is_some() {
                    h += 4; // Pipeline/status detail when memory is active.
                }
            }
            h.max(12)
        }
...
fn render_single_widget(frame: &mut Frame, placement: &WidgetPlacement, data: &InfoWidgetData) {
    ...
    } else if placement.kind == WidgetKind::ModelInfo {
        block = block.title(Span::styled(
            " IL Stats ",
            Style::default().fg(rgb(120, 160, 255)).dim(),
        ));
    }
