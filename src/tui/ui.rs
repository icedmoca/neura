...
fn draw_inner(frame: &mut Frame, app: &dyn TuiState) {
    ...
    // Draw info widget overlays. Keep the IL/model stats widget visible even during idle
    // animation so the right-side stats box remains anchored instead of blinking out.
