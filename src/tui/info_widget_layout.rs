...
const MIN_WIDGET_WIDTH: u16 = 28;
/// Maximum width the widget can take.
const MAX_WIDGET_WIDTH: u16 = 48;
...
pub(crate) fn calculate_placements(
    ...
    let model_info_requested = available.contains(&WidgetKind::ModelInfo);
    ...
    if model_info_requested && !placements.iter().any(|p| p.kind == WidgetKind::ModelInfo) {
        let width = messages_area.width.min(MAX_WIDGET_WIDTH).max(1);
        let height =
            calculate_widget_height(WidgetKind::ModelInfo, data, width, messages_area.height)
