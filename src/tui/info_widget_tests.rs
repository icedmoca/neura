use super::{
    ...
    MemoryEventKind, MemoryInfo, MemoryState, PipelineState, Side, StepStatus, SwarmInfo,
...
fn workspace_widget_has_high_priority_when_enabled() {
    ...
    assert_eq!(available.first(), Some(&WidgetKind::ModelInfo));
    ...
    assert_eq!(
        ...
        Some(WidgetKind::ModelInfo)
    );
    assert_eq!(placements.first().map(|p| p.side), Some(Side::Right));
}
