    ...
    match code {
        ...
        KeyCode::Enter => {
            ...
            if !app.input.is_empty() {
                ...
                if app_mod::commands::handle_exit_command(app, trimmed) {
                    return Ok(());
                }
                ...
                if trimmed == "/reload" {
                    ...
                    if client_needs_reload {
                        ...
                        // Do not persist `/reload` itself as restored input after the
                        // client execs into the newer binary.
                        app.input.clear();
                        app.cursor_pos = 0;
                ...
                if trimmed == "/client-reload" {
                    ...
                    let session_id = app
                        ...
                        .clone()
                        .unwrap_or_else(|| crate::id::new_id("ses"));
                    // Do not restore `/client-reload` into the input box after restart.
                    app.input.clear();
                    app.cursor_pos = 0;
                    app.save_input_for_reload(&session_id);
                    app.reload_requested = Some(session_id);
                ...
                if let Some(command) = app_mod::commands::parse_enrich_command(trimmed) {
                    match command {
                        Err(error) => app.push_display_message(DisplayMessage::error(error)),
                        Ok(command) => {
                            let message = app_mod::commands::enrich_command_message(command);
