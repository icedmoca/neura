...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum EnrichCommand {
    On,
    Off,
    Status,
}

pub(super) fn parse_enrich_command(trimmed: &str) -> Option<Result<EnrichCommand, String>> {
    match trimmed {
        "/enrich" | "/enrich status" => Some(Ok(EnrichCommand::Status)),
        "/enrich on" => Some(Ok(EnrichCommand::On)),
        "/enrich off" => Some(Ok(EnrichCommand::Off)),
        _ if trimmed.starts_with("/enrich ") => Some(Err(
            "Usage: `/enrich on`, `/enrich off`, or `/enrich status`.".to_string(),
        )),
        _ => None,
    }
}

pub(super) fn enrich_command_message(command: EnrichCommand) -> String {
    match command {
        EnrichCommand::On => {
            crate::local_model::set_enrich_enabled(true);
            crate::local_model::enrich_status_message()
        }
        EnrichCommand::Off => {
            crate::local_model::set_enrich_enabled(false);
            crate::local_model::enrich_status_message()
        }
        EnrichCommand::Status => crate::local_model::enrich_status_message(),
    }
}
...
pub(super) fn handle_exit_command(app: &mut App, trimmed: &str) -> bool {
    if trimmed == "/exit" {
        app.exit_now();
        return true;
    }
...
pub(super) fn handle_session_command(app: &mut App, trimmed: &str) -> bool {
    ...
    if let Some(command) = parse_enrich_command(trimmed) {
        match command {
            Err(error) => app.push_display_message(DisplayMessage::error(error)),
            Ok(command) => {
                let message = enrich_command_message(command);
                app.set_status_notice(if crate::local_model::enrich_enabled() {
                    "Enrich: ON"
                } else {
                    "Enrich: OFF"
                });
