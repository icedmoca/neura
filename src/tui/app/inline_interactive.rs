...
impl App {
    ...
    pub(super) fn open_model_picker(&mut self) {
        ...
        let mut routes: Vec<crate::provider::ModelRoute> = if self.is_remote {
    ...
    fn push_local_quantized_model_route(routes: &mut Vec<crate::provider::ModelRoute>) {
        if routes
            .iter()
            .any(|route| route.api_method == "local-bridge")
        {
            return;
        }
        for model in crate::local_model::model_ids().into_iter().rev() {
            routes.insert(
                0,
    ...
    fn open_model_picker_with_routes(
        ...
        entries.sort_by(|a, b| {
            let a_local = if a
                .options
                .iter()
                .any(|r| r.provider == "Local Models" || r.api_method == "local-bridge")
            {
                0u8
            } else {
                1
