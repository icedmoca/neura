    ...
    app.session_picker_overlay = Some(RefCell::new(
        crate::tui::session_picker::SessionPicker::new(vec![
            crate::tui::session_picker::SessionInfo {
                ...
                server_name: None,
                server_icon: None,
                source: crate::tui::session_picker::SessionSource::Neura,
                resume_target: crate::tui::session_picker::ResumeTarget::NeuraSession {
...
#[test]
fn test_exit_command_quits_immediately() {
    let mut app = create_test_app();
    ...
    assert!(matches!(app.session.status, crate::session::SessionStatus::Closed));
}
...
fn test_git_command_works_in_remote_mode_with_accessible_working_directory() {
    ...
    assert!(
        !msg.content
            .contains("currently only available in a local neura TUI session")
    );
}
