#[test]
fn test_model_picker_always_shows_local_quantized_model_in_remote_mode() {
    ...
    let local_entry = picker
        .entries
        .iter()
        .find(|entry| entry.name == crate::local_model::LOCAL_MODEL_ID)
        .expect("local quantized GGUF model should be visible");
    assert!(local_entry.options.iter().any(|route| {
        route.provider == "Local Models" && route.api_method == "local-bridge"
    }));
}
