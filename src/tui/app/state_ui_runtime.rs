...
impl App {
    ...
    pub(super) fn build_turn_footer(&self, duration: Option<f32>) -> Option<String> {
        ...
        if let Some(il) = format_interlang_footer() {
            parts.push(il);
        }
...
fn format_interlang_footer() -> Option<String> {
    ...
    let mode = status.get("mode").and_then(|v| v.as_str()).unwrap_or("il");
    let last = status
