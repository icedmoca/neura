...
pub(super) fn handle_dev_command(app: &mut App, trimmed: &str) -> bool {
    if trimmed == "/reload" {
        ...
        // Do not persist the lifecycle command itself as restored input. Otherwise the
        // resumed TUI can come back with `/reload` still in the input box, making reload
        // look like it glitched or allowing an accidental second reload.
        app.input.clear();
        app.cursor_pos = 0;
    ...
    if trimmed == "/restart" {
        ...
        // Do not restore `/restart` into the input box after the process comes back.
        app.input.clear();
        app.cursor_pos = 0;
        app.session.provider_session_id = app.provider_session_id.clone();
        app.session
