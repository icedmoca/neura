...
const MEMORY_PROMPT_TOKEN_BUDGET: usize = 900;
...
fn selected_entries_for_prompt_budgeted(
    entries: &[MemoryEntry],
    limit: usize,
) -> Vec<&MemoryEntry> {
    let mut selected = Vec::new();
    ...
    for entry in entries.iter().filter(|e| e.active) {
        ...
        let line = format!("{}. {}\n", selected.len() + 1, entry.content.trim());
        let tokens =
            crate::interlang::exact_token_count(&line).unwrap_or_else(|| line.len().div_ceil(4));
        if !selected.is_empty() && used_tokens + tokens > MEMORY_PROMPT_TOKEN_BUDGET {
            break;
        }
        used_tokens += tokens;
        selected.push(entry);
...
pub(super) fn format_entries_for_prompt(entries: &[MemoryEntry], limit: usize) -> Option<String> {
    let mut sections: HashMap<MemoryCategory, Vec<&MemoryEntry>> = HashMap::new();
    let selected_entries = selected_entries_for_prompt_budgeted(entries, limit);
    ...
    let relation_tags = selected_entries
        .iter()
        .flat_map(|entry| entry.tags.iter())
        .filter(|tag| tag.starts_with("rel:"))
        .cloned()
        .collect::<std::collections::BTreeSet<_>>();
    if !relation_tags.is_empty() {
        output.push_str("\n## Memory relation hints\n");
        for tag in relation_tags.into_iter().take(8) {
            output.push_str(&format!("- {}\n", tag.trim_start_matches("rel:")));
        }
    }
