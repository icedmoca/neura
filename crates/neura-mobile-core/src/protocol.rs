...
/// Requests sent by the mobile app to the neura gateway.
...
/// Events received by the mobile app from the neura gateway.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
...
mod tests {
    ...
    fn history_payload_decodes_server_metadata() {
        let event: Result<MobileServerEvent, _> = serde_json::from_value(
            json!({"type":"history","session_id":"s1","server_name":"neura","provider_model":"gpt-5","available_models":["gpt-5","claude-sonnet-4"],"all_sessions":["s1","s2"],"messages":[{"role":"assistant","content":"hi"}]}),
        );
