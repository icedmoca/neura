...
impl Default for PairingForm {
    fn default() -> Self {
        Self {
            ...
            device_name: "neura simulator".to_string(),
        }
    }
}
    ...
    pub fn for_scenario(scenario: ScenarioName) -> Self {
        match scenario {
            ScenarioName::Onboarding => Self {
                ...
                status_message: Some("Ready to pair with a neura server.".to_string()),
                error_message: None,
                messages: Vec::new(),
            ...
            ScenarioName::ConnectedChat => {
                ...
                Self {
                    ...
                    status_message: Some("Connected to simulated neura server.".to_string()),
                    error_message: None,
                    messages: vec![
                        ChatMessage {
            ...
            ScenarioName::ToolApprovalRequired => {
                ...
                state.messages.push(ChatMessage {
                    ...
                    text: "Tool approval required: bash: cargo test -p neura-mobile-core."
                        .to_string(),
                });
            ...
            ScenarioName::NetworkReconnect => {
                ...
                state.status_message =
                    Some("Reconnecting to simulated neura server...".to_string());
                state
            }
    ...
    pub fn dispatch(&mut self, action: SimulatorAction) -> DispatchReport {
        ...
        while let Some(action) = pending.pop() {
            ...
            for effect in reduction.effects {
                ...
                let follow_ups = FakeNeuraBackend::default().handle_effect(effect);
...
fn reduce(mut state: SimulatorState, action: SimulatorAction) -> Reduction {
    ...
    match action {
        ...
        SimulatorAction::Connected { session_id } => {
            ...
            state.status_message = Some("Connected to simulated neura server.".to_string());
            state.error_message = None;
...
pub struct FakeNeuraBackend;

impl FakeNeuraBackend {
