    ...
    fn empty_sessions() -> Self {
        Self {
            ...
            surfaces: vec![Surface {
                id: 1,
                title: "no neura sessions found".to_string(),
                body_lines: vec![
    ...
    pub fn status_title(&self) -> String {
        ...
        match self.mode {
            InputMode::Navigation if self.zoomed => format!(
                "Neura Desktop · {mode}{zoom} · workspace {workspace} · panel {panel_size} · {focused} · j/k scroll · g/G top/bottom · z unzoom · o/Enter open · Esc quit"
            ),
            InputMode::Navigation => format!(
                "Neura Desktop · {mode}{zoom} · workspace {workspace} · panel {panel_size} · {focused} · h/l columns · j/k workspaces · Ctrl+1-4 panel size · Ctrl+R refresh · Ctrl+; new · Ctrl+? help · z zoom · i insert · Esc quit"
            ),
            InputMode::Insert => {
                format!(
                    "Neura Desktop · {mode}{zoom} · workspace {workspace} · {focused} · Ctrl+Enter send · Enter newline · Esc NAV"
                )
            }
        }
    }
