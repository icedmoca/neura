                    ...
                    if event.state == ElementState::Pressed =>
                {
                    ...
                    match app.handle_key(key_input) {
                        ...
                        KeyOutcome::OpenSession { session_id, title } => {
                            ...
                            {
                                eprintln!(
                                    "neura-desktop: failed to open session {session_id}: {error:#}"
                                );
                            }
                        }
                        KeyOutcome::SpawnSession => {
                            ...
                            if let Err(error) = session_launch::launch_new_session() {
                                eprintln!("neura-desktop: failed to spawn session: {error:#}");
                            } else {
                        ...
                        KeyOutcome::SendDraft {
                            ...
                            if app.is_single_session() {
                                ...
                                eprintln!(
                                    "neura-desktop: failed to send draft to {session_id}: {error:#}"
                                );
                            } else {
                                std::thread::sleep(SESSION_SPAWN_REFRESH_DELAY);
                                app.refresh_sessions();
            ...
            Event::AboutToWait => {
                ...
                if let Some(relaunch) = hot_reloader.poll() {
                    if let Err(error) = relaunch.spawn() {
                        eprintln!("neura-desktop: failed to hot reload desktop: {error:#}");
                    } else {
                        target.exit();
                        return;
                    }
...
fn load_session_cards_for_desktop() -> Vec<workspace::SessionCard> {
    match session_data::load_recent_session_cards() {
        Ok(cards) => cards,
        Err(error) => {
            eprintln!("neura-desktop: failed to load session metadata: {error:#}");
            Vec::new()
        }
    }
}
...
fn load_desktop_preferences() -> Option<workspace::DesktopPreferences> {
    match desktop_prefs::load_preferences() {
        ...
        Err(error) => {
            eprintln!("neura-desktop: failed to load desktop preferences: {error:#}");
            None
        }
    }
}
...
fn save_desktop_preferences(workspace: &Workspace) {
    if let Err(error) = desktop_prefs::save_preferences(&workspace.preferences()) {
        eprintln!("neura-desktop: failed to save desktop preferences: {error:#}");
    }
}
...
impl<'window> Canvas<'window> {
    async fn new(window: &'window Window) -> Result<Self> {
        ...
        let (device, queue) = adapter
            .request_device(
                &wgpu::DeviceDescriptor {
                    label: Some("neura-desktop-device"),
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("neura-desktop-primitive-shader"),
            source: wgpu::ShaderSource::Wgsl(SHADER.into()),
        });
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("neura-desktop-primitive-pipeline-layout"),
            bind_group_layouts: &[],
            push_constant_ranges: &[],
        });
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("neura-desktop-primitive-pipeline"),
    ...
    fn render(
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("neura-desktop-render-workspace"),
            });
        let now = Instant::now();
        ...
        let vertex_buffer = self
            ...
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("neura-desktop-workspace-vertices"),
        ...
        if !text_areas.is_empty() {
            if let Err(error) = self.text_renderer.prepare(
                ...
                eprintln!("neura-desktop: failed to prepare text: {error:?}");
            }
        }

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("neura-desktop-workspace-pass"),
            ...
            {
                eprintln!("neura-desktop: failed to render text: {error:?}");
            }
        }
