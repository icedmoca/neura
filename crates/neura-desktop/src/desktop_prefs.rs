...
fn preferences_path() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("NEURA_DESKTOP_STATE") {
        return Ok(PathBuf::from(path));
    }

    if let Ok(path) = std::env::var("NEURA_HOME") {
        return Ok(PathBuf::from(path).join("config/neura/desktop-state.json"));
    }

    if let Ok(path) = std::env::var("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(path).join("neura/desktop-state.json"));
    }

    let home = std::env::var_os("HOME")
        .map(PathBuf::from)
        .context("HOME is not set")?;
    Ok(home.join(".config/neura/desktop-state.json"))
}
...
mod tests {
    ...
    fn saves_and_loads_preferences() -> Result<()> {
        ...
        let dir =
            std::env::temp_dir().join(format!("neura-desktop-prefs-test-{}", std::process::id()));
        let path = dir.join("state.json");
        unsafe {
            std::env::set_var("NEURA_DESKTOP_STATE", &path);
        }
        ...
        unsafe {
            std::env::remove_var("NEURA_DESKTOP_STATE");
        }
