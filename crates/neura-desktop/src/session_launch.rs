...
pub fn launch_resume_session(session_id: &str, title: &str) -> Result<()> {
    let title = format!("neura · {}", compact_title(title));
    let candidates = terminal_candidates(&title, &["--resume", session_id]);
    launch_first_available_terminal(candidates, &format!("neura --resume {session_id}"))
}

pub fn launch_new_session() -> Result<()> {
    let candidates = terminal_candidates("neura · new session", &["--fresh-spawn"]);
    launch_first_available_terminal(candidates, "neura")
}
...
pub fn send_message_to_session(session_id: &str, _title: &str, message: &str) -> Result<()> {
    ...
    Command::new(neura_bin())
        ...
        .stderr(Stdio::null())
        .spawn()
        .with_context(|| format!("failed to spawn neura run for {session_id}"))?;
...
pub fn spawn_fresh_server_session(
    ...
    std::thread::Builder::new()
        .name("neura-desktop-fresh-session".to_string())
        .spawn(move || {
            if let Err(error) =
...
pub fn spawn_message_to_session(
    ...
    let handle = DesktopSessionHandle { command_tx };
    std::thread::Builder::new()
        .name("neura-desktop-session-message".to_string())
        .spawn(move || {
...
fn ensure_server_running() -> Result<()> {
    ...
    Command::new(neura_bin())
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to spawn neura serve")?;
...
fn connect_server_with_retry_path(socket_path: &PathBuf, timeout: Duration) -> Result<UnixStream> {
    ...
    match last_error {
        Some(error) => Err(error).with_context(|| {
            format!(
                "timed out connecting to neura server at {}",
                socket_path.display()
            )
        }),
        None => anyhow::bail!("timed out connecting to neura server"),
    }
}
...
fn read_session_id_from_events(
    ...
    while started.elapsed() < timeout {
        ...
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("neura server disconnected before assigning a session"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse neura server event")?;
                if value.get("type").and_then(Value::as_str) == Some("session") {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("neura server sent malformed session event");
                    };
                ...
                if value.get("type").and_then(Value::as_str) == Some("error") {
                    ...
                    anyhow::bail!("neura server rejected fresh session: {message}");
                }
            ...
            Err(error) => return Err(error).context("failed to read neura server event"),
        }
    }

    anyhow::bail!("timed out waiting for neura server session id")
}
...
fn read_session_id_from_state(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("neura server disconnected before returning state"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse neura server event")?;
                ...
                {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("neura server sent malformed state event");
                    };
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(state_request_id)
                {
                    let message = value
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or("unknown server error");
                    anyhow::bail!("neura server rejected state request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for neura server state")
}
...
fn socket_path() -> PathBuf {
    ...
    if let Ok(dir) = std::env::var("NEURA_RUNTIME_DIR") {
        return PathBuf::from(dir).join("neura.sock");
    }
    if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
        return PathBuf::from(dir).join("neura.sock");
    }
    std::env::temp_dir()
        .join(format!("neura-{}", runtime_user_discriminator()))
        .join("neura.sock")
}
...
fn terminal_candidates(title: &str, neura_args: &[&str]) -> Vec<Command> {
    let mut candidates = Vec::new();

    if let Ok(program) = std::env::var("NEURA_DESKTOP_TERMINAL") {
        candidates.push(terminal_command(program, &[], neura_args));
    }
    ...
    candidates.push(terminal_command("foot", &["-T", title, "--"], neura_args));
    candidates.push(terminal_command("kitty", &["--title", title], neura_args));
    candidates.push(terminal_command(
        "alacritty",
        &["-t", title, "-e"],
        neura_args,
    ));
    candidates.push(terminal_command("wezterm", &["start", "--"], neura_args));
    candidates.push(terminal_command(
        "x-terminal-emulator",
        &["-T", title, "-e"],
        neura_args,
...
fn neura_bin() -> String {
    std::env::var("NEURA_BIN").unwrap_or_else(|_| "neura".to_string())
}
...
mod tests {
    ...
    fn desktop_event_parser_maps_streaming_server_events() {
        ...
        assert_eq!(
            desktop_event_from_server_value(&json!({
                "type": "reloading",
                "new_socket": "/tmp/neura-new.sock"
            })),
            Some(DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/neura-new.sock".to_string())
            })
        );
    }
    ...
    fn desktop_worker_roundtrips_message_with_fake_server() -> Result<()> {
        ...
        let socket_path = std::env::temp_dir().join(format!(
            "neura-desktop-worker-smoke-{}-{}.sock",
            std::process::id(),
        ...
        let previous_socket = std::env::var_os("NEURA_SOCKET");
        unsafe {
            std::env::set_var("NEURA_SOCKET", &socket_path);
        }
        ...
        restore_env_var("NEURA_SOCKET", previous_socket);
        let _ = std::fs::remove_file(&socket_path);
