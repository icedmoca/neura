...
pub fn default_socket_path() -> PathBuf {
    runtime_dir().join("neura-mobile-sim.sock")
}
...
pub async fn run_server(socket_path: &Path, initial_scenario: ScenarioName) -> Result<()> {
    ...
    let store = Arc::new(Mutex::new(SimulatorStore::new(
        neura_mobile_core::SimulatorState::for_scenario(initial_scenario),
    )));
...
pub async fn run_server(_socket_path: &Path, _initial_scenario: ScenarioName) -> Result<()> {
    bail!("neura-mobile-sim currently supports Unix socket automation only")
}
    ...
    let result = match request.method.as_str() {
        ...
        "assert_replay" => {
            ...
            match trace_value.and_then(|value| {
                serde_json::from_value::<neura_mobile_core::ReplayTrace>(value).map_err(Into::into)
            }) {
...
fn text_action_for_node(
    state: &neura_mobile_core::SimulatorState,
    node_id: &str,
    text: &str,
    append: bool,
...
fn runtime_dir() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("neura-mobile-sim-{}", user_discriminator()))
}
