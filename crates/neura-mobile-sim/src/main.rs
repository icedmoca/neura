...
#[derive(Parser, Debug)]
#[command(name = "neura-mobile-sim")]
#[command(about = "Headless-first mobile simulator and automation CLI for neura")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Preview { socket, scenario } => {
            ...
            neura_mobile_sim::gpu_preview::run_preview(scene)
        }
        Command::PreviewMesh {
            socket,
            scenario,
            ...
            let scene = resolve_preview_scene(socket, &scenario).await?;
            let mesh = neura_mobile_sim::gpu_preview::build_preview_mesh(&scene);
