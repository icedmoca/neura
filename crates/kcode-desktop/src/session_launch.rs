...
pub fn launch_resume_session(session_id: &str, title: &str) -> Result<()> {
    let title = format!("kcode · {}", compact_title(title));
    let candidates = terminal_candidates(&title, &["--resume", session_id]);
    launch_first_available_terminal(candidates, &format!("kcode --resume {session_id}"))
}

pub fn launch_new_session() -> Result<()> {
    let candidates = terminal_candidates("kcode · new session", &["--fresh-spawn"]);
    launch_first_available_terminal(candidates, "kcode")
}
...
pub fn send_message_to_session(session_id: &str, _title: &str, message: &str) -> Result<()> {
    ...
    Command::new(kcode_bin())
        ...
        .stderr(Stdio::null())
        .spawn()
        .with_context(|| format!("failed to spawn kcode run for {session_id}"))?;
...
pub fn spawn_fresh_server_session(
    ...
    std::thread::Builder::new()
        .name("kcode-desktop-fresh-session".to_string())
        .spawn(move || {
            if let Err(error) =
...
pub fn spawn_message_to_session(
    ...
    let handle = DesktopSessionHandle { command_tx };
    std::thread::Builder::new()
        .name("kcode-desktop-session-message".to_string())
        .spawn(move || {
...
fn ensure_server_running() -> Result<()> {
    ...
    Command::new(kcode_bin())
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to spawn kcode serve")?;
...
fn connect_server_with_retry_path(socket_path: &PathBuf, timeout: Duration) -> Result<UnixStream> {
    ...
    match last_error {
        Some(error) => Err(error).with_context(|| {
            format!(
                "timed out connecting to kcode server at {}",
                socket_path.display()
            )
        }),
        None => anyhow::bail!("timed out connecting to kcode server"),
    }
}
...
fn read_session_id_from_events(
    ...
    while started.elapsed() < timeout {
        ...
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("kcode server disconnected before assigning a session"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse kcode server event")?;
                if value.get("type").and_then(Value::as_str) == Some("session") {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("kcode server sent malformed session event");
                    };
                ...
                if value.get("type").and_then(Value::as_str) == Some("error") {
                    ...
                    anyhow::bail!("kcode server rejected fresh session: {message}");
                }
            ...
            Err(error) => return Err(error).context("failed to read kcode server event"),
        }
    }

    anyhow::bail!("timed out waiting for kcode server session id")
}
...
fn read_session_id_from_state(
    ...
    while started.elapsed() < timeout {
        line.clear();
        match reader.read_line(&mut line) {
            Ok(0) => anyhow::bail!("kcode server disconnected before returning state"),
            Ok(_) => {
                let value: Value = serde_json::from_str(line.trim())
                    .context("failed to parse kcode server event")?;
                ...
                {
                    let Some(session_id) = value.get("session_id").and_then(Value::as_str) else {
                        anyhow::bail!("kcode server sent malformed state event");
                    };
                    ...
                    && value.get("id").and_then(Value::as_u64) == Some(state_request_id)
                {
                    let message = value
                        ...
                        .and_then(Value::as_str)
                        .unwrap_or("unknown server error");
                    anyhow::bail!("kcode server rejected state request: {message}");
                }
            }
            Err(error)
                if matches!(
    ...
    anyhow::bail!("timed out waiting for kcode server state")
}
...
fn socket_path() -> PathBuf {
    ...
    if let Ok(dir) = std::env::var("KCODE_RUNTIME_DIR") {
        return PathBuf::from(dir).join("kcode.sock");
    }
    if let Ok(dir) = std::env::var("XDG_RUNTIME_DIR") {
        return PathBuf::from(dir).join("kcode.sock");
    }
    std::env::temp_dir()
        .join(format!("kcode-{}", runtime_user_discriminator()))
        .join("kcode.sock")
}
...
fn terminal_candidates(title: &str, kcode_args: &[&str]) -> Vec<Command> {
    let mut candidates = Vec::new();

    if let Ok(program) = std::env::var("KCODE_DESKTOP_TERMINAL") {
        candidates.push(terminal_command(program, &[], kcode_args));
    }
    ...
    candidates.push(terminal_command("foot", &["-T", title, "--"], kcode_args));
    candidates.push(terminal_command("kitty", &["--title", title], kcode_args));
    candidates.push(terminal_command(
        "alacritty",
        &["-t", title, "-e"],
        kcode_args,
    ));
    candidates.push(terminal_command("wezterm", &["start", "--"], kcode_args));
    candidates.push(terminal_command(
        "x-terminal-emulator",
        &["-T", title, "-e"],
        kcode_args,
...
fn kcode_bin() -> String {
    std::env::var("KCODE_BIN").unwrap_or_else(|_| "kcode".to_string())
}
...
mod tests {
    ...
    fn desktop_event_parser_maps_streaming_server_events() {
        ...
        assert_eq!(
            desktop_event_from_server_value(&json!({
                "type": "reloading",
                "new_socket": "/tmp/kcode-new.sock"
            })),
            Some(DesktopSessionEvent::Reloading {
                new_socket: Some("/tmp/kcode-new.sock".to_string())
            })
        );
    }
    ...
    fn desktop_worker_roundtrips_message_with_fake_server() -> Result<()> {
        ...
        let socket_path = std::env::temp_dir().join(format!(
            "kcode-desktop-worker-smoke-{}-{}.sock",
            std::process::id(),
        ...
        let previous_socket = std::env::var_os("KCODE_SOCKET");
        unsafe {
            std::env::set_var("KCODE_SOCKET", &socket_path);
        }
        ...
        restore_env_var("KCODE_SOCKET", previous_socket);
        let _ = std::fs::remove_file(&socket_path);
